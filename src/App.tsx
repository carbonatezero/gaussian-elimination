...
export default function App() {
  ...
  const [normalizePivots, setNormalizePivots] = useState(false);
  ...
  function start() {
    ...
    try {
      const A = parseMatrixStrings(editor);
            const { steps: s } = generateSteps(A, { normalizePivots });
      setSteps(s);
      setStepIndex(0);
    } catch (e: any) {
      setSteps(null);
            ...
            {METHOD.map((m, i) => {
              ...
              return (
                <div key={i} className={"methodItem " + (active ? "methodItemActive" : "")}>
                  <div className="methodIndex">STEP {i + 1}</div>
          ...
          <div className="panelBody">
            <div className="controls">
              ...
              <label className="badge" style={{ cursor: "pointer" }}>
                <input
                  type="checkbox"
                  checked={normalizePivots}
                  onChange={(e) => setNormalizePivots(e.target.checked)}
                  style={{ marginRight: 8 }}
                />
              ...
              <div style={{ marginTop: 12 }} className="explainer">
                {currentStep.rowOpText.trim().length > 0 && (
                  <div style={{ marginBottom: 8 }}>
                    Row operation: <span className="op"><MathInline latex={currentStep.rowOpText} /></span>
                  </div>
                )}
                ...
                {logItems.map((it) => {
                  ...
                  return (
                    <div key={it.i} className={"logItem " + (active ? "logItemActive" : "")}>
                      <span className="small">Step {it.i + 1}:</span> <span className="op"><MathInline latex={it.txt} /></span>
                    </div>
                  );
                })}
              </div>
